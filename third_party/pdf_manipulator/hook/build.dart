...
void main(List<String> args) async {
  await build(args, (BuildInput input, BuildOutputBuilder output) async {
    ...
    // Flutter 3.44 requests a bundled dylib for iOS, while the upstream
    // release deliberately publishes static iOS archives. Resolve and verify
    // that archive, then wrap its objects in a normal @rpath dylib so the
    // emitted CodeAsset still honors Flutter's requested link preference.
    if (codeConfig.targetOS == OS.iOS && linkMode is DynamicLoadingBundled) {
      final staticMode = StaticLinking();
      final staticName = codeConfig.targetOS.libraryFileName(
        _crateName,
        staticMode,
      );
      final staticFile = File.fromUri(
        input.outputDirectory.resolve(staticName),
      );
      await _resolveNative(
        packageRoot: input.packageRoot,
        version: readVersion(input.packageRoot),
        targetKey: _targetKey(codeConfig),
        libFileName: staticName,
        dest: staticFile,
        cacheFile: File.fromUri(
          input.outputDirectoryShared.resolve('$targetTriple/$staticName'),
        ),
        targetTriple: targetTriple,
        linkMode: staticMode,
        buildInput: input,
      );
      await _linkIosArchiveAsDylib(codeConfig, staticFile, outFile);
    } else {
      await _resolveNative(
        packageRoot: input.packageRoot,
        version: readVersion(input.packageRoot),
...
Future<void> _linkIosArchiveAsDylib(
  CodeConfig code,
  File archive,
  File output,
) async {
  ...
  if (sdkResult.exitCode != 0 || '${sdkResult.stdout}'.trim().isEmpty) {
    throw StateError('Could not locate the $sdkName SDK: ${sdkResult.stderr}');
  }
  final compiler = code.cCompiler?.compiler;
  if (compiler == null) {
    throw StateError('Flutter did not provide an Apple C compiler');
  }
  ...
  final minimumFlag = simulator
      ? '-mios-simulator-version-min=${code.iOS.targetVersion}'
      : '-miphoneos-version-min=${code.iOS.targetVersion}';
  final result = await Process.run(File.fromUri(compiler).path, [
    '-arch',
    architecture,
    '-isysroot',
    ...
    '-Wl,-install_name,@rpath/${output.uri.pathSegments.last}',
    '-framework',
    'Security',
    '-framework',
    'CoreFoundation',
  ...
  if (result.exitCode != 0) {
    throw StateError('Could not link iOS PDF engine: ${result.stderr}');
  }
}
