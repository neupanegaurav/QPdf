...
void main() {
  ...
  testWidgets('Fill & Sign is prominent from home and inside the editor', (
    ...
    await tester.pumpAndSettle();
    expect(find.text('Smart Fill'), findsOneWidget);
  ...
  testWidgets('Smart Fill asks once and populates matching PDF fields', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1600, 1000));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    final source = PdfDocumentSource(
      id: 'smart-form-workflow',
      displayName: 'Smart Application.pdf',
      bytes: formEditor.save(),
    );
    await tester.pumpWidget(
      QPdfApp(
        engine: _FakeEngine(),
        fileService: _FakeFileService(source, '/tmp/Smart Application.pdf'),
        recoveryService: const NoopDocumentRecoveryService(),
      ),
    );
    ...
    await tester.tap(find.byKey(const Key('fill-and-sign-toolbar-button')));
    await tester.pumpAndSettle();
    await tester.tap(find.byKey(const Key('smart-fill-form-button')));
    await tester.pumpAndSettle();

    expect(find.text('Smart Fill found 1 field'), findsOneWidget);
    expect(find.textContaining('Nothing leaves this device'), findsOneWidget);
    expect(find.textContaining('Applicant email'), findsOneWidget);
    await tester.enterText(find.byType(TextFormField), 'person@example.com');
    await tester.tap(find.byKey(const Key('smart-fill-apply-button')));
    await tester.pumpAndSettle();
    ...
    final controller = tester
        ...
        .controller!;
    expect(
      controller.acroForm?.fieldNamed('applicant_email')?.value,
      'person@example.com',
    );
  });

  testWidgets('Smart Fill reveals conditional spouse fields on demand', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1600, 1000));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    final formEditor = PdfEditor(PdfDocument.open(PdfBlankDocument.create()));
    formEditor.addCheckBoxField(
      0,
      'has_spouse',
      const PdfRect(72, 680, 96, 704),
    );
    formEditor.addTextField(
      0,
      'spouse_full_name',
      const PdfRect(110, 680, 340, 716),
    );
    formEditor.addTextField(
      0,
      'applicant_name',
      const PdfRect(72, 620, 340, 656),
    );
    formEditor.acroForm!.dict['XFA'] = CosString.fromText(
      'synthetic compatibility marker',
    );
    final source = PdfDocumentSource(
      id: 'conditional-smart-form',
      displayName: 'Conditional Application.pdf',
      bytes: formEditor.save(),
    );
    await tester.pumpWidget(
      QPdfApp(
        ...
        fileService: _FakeFileService(
          source,
          '/tmp/Conditional Application.pdf',
        ),
        recoveryService: const NoopDocumentRecoveryService(),
      ),
    );
    ...
    expect(find.byKey(const Key('smart-fill-conditional-summary')), findsOne);
    expect(
      find.byKey(const Key('smart-fill-compatibility-warning')),
      findsOneWidget,
    );
    expect(find.byKey(const Key('smart-fill-focus-order')), findsOneWidget);
    expect(
      tester
          .getSemantics(find.byKey(const Key('smart-fill-conditional-summary')))
          .flagsCollection
          .isLiveRegion,
      isTrue,
    );
    expect(
      tester
          .getSemantics(find.text('Personal details').first)
          .flagsCollection
          .isHeader,
      isTrue,
    );
    expect(
      find.byKey(const Key('smart-form-field-spouse_full_name')),
      findsNothing,
    );
    await tester.tap(find.byKey(const Key('smart-form-field-has_spouse')));
    await tester.pumpAndSettle();
    expect(
      find.byKey(const Key('smart-form-field-spouse_full_name')),
      findsOneWidget,
    ...
    expect(
      find.byKey(const Key('smart-fill-conditional-summary')),
      findsNothing,
    );
  });

  testWidgets('Smart Fill warns for an official hybrid XFA form', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1600, 1000));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    final source = PdfDocumentSource(
      id: 'official-w9',
      displayName: 'IRS Form W-9.pdf',
      bytes: File(
        '${Directory.current.path}/../../output/pdf/c4-real-world/irs-w9.pdf',
      ).readAsBytesSync(),
    );
    await tester.pumpWidget(
      QPdfApp(
        engine: _FakeEngine(),
        fileService: _FakeFileService(source, '/tmp/IRS Form W-9.pdf'),
        recoveryService: const NoopDocumentRecoveryService(),
      ),
    );
    ...
    expect(find.textContaining('Adobe XFA form'), findsOneWidget);
    expect(find.text('Name of entity/individual'), findsWidgets);
    expect(
      find.byKey(
        const Key('smart-form-label-topmostSubform[0].Page1[0].f1_01[0]'),
      ),
      findsOneWidget,
    );
    expect(find.textContaining('95% confidence'), findsWidgets);
      ...
      tester
          .widget<InputDecorator>(
            find.descendant(
              of: find.byKey(
                const Key(
                  'smart-form-field-topmostSubform[0].Page1[0].f1_01[0]',
                ),
              ),
  ...
  testWidgets('Smart Fill detects and converts native flat-form controls', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1600, 1000));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    final flat = pw.Document();
    flat.addPage(
      pw.Page(
    ...
    final source = PdfDocumentSource(
      id: 'flat-form-workflow',
      displayName: 'Flat Application.pdf',
      bytes: await flat.save(),
    );
    await tester.pumpWidget(
      QPdfApp(
        engine: _FakeEngine(),
        fileService: _FakeFileService(source, '/tmp/Flat Application.pdf'),
        recoveryService: const NoopDocumentRecoveryService(),
      ),
    );
    ...
    expect(find.text('Review detected fields'), findsOneWidget);
    expect(find.textContaining('Full name'), findsOneWidget);
    expect(find.textContaining('I agree to the terms'), findsOneWidget);
    await tester.tap(find.byKey(const Key('create-detected-fields-button')));
    await tester.pumpAndSettle();

    expect(find.text('Smart Fill found 2 fields'), findsOneWidget);
    await tester.enterText(find.byType(TextFormField), 'Casey Example');
    ...
    final controller = tester
        ...
        .controller!;
    expect(
      controller.acroForm?.fieldNamed('Full name')?.value,
      'Casey Example',
    );
    expect(
      controller.acroForm?.fieldNamed('I agree to the terms')?.isChecked,
      isTrue,
    );
  });
