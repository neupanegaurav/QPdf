...
Read, annotate, organize, fill, sign, scan, and OCR PDFs privately on device.
...
redaction creates an image-only copy and removes searchable text, interactive
content, metadata, and existing signatures.

No account is required. QPdf currently includes no advertising or analytics
SDK and does not upload opened PDFs.
